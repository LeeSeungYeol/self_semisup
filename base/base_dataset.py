...
class BaseDataSet_SubCls(Dataset):
    def __init__(self, data_dir, split, mean, std, ignore_index, base_size=None, augment=True, val=False,
                 jitter=False, use_weak_lables=False, weak_labels_output=None, crop_size=None, scale=False, flip=False,
                 rotate=False,
                 blur=False, return_id=False, n_labeled_examples=None):
    ...
    def _augmentation_tri(self, image, label, label2):
        h, w, _ = image.shape

        if self.base_size is not None:
            # image, label = self._resize(image, label)
            image, label, label2 = self._resize_tri(image, label, label2)

        if self.crop_size is not None:
            # image, label = self._crop(image, label)
            image, label, label2 = self._crop_tri(image, label, label2)

        if self.flip:
            # image, label = self._flip(image, label)
            image, label, label2 = self._flip_tri(image, label, label2)

        image = Image.fromarray(np.uint8(image))
        image = self.jitter_tf(image) if self.jitter else image

        return self.normalize(self.to_tensor(image)), label, label2

    def _resize_tri(self, image, label, label2, bigger_side_to_base_size=True):
        if isinstance(self.base_size, int):
            h, w, _ = image.shape
            if self.scale:
                longside = random.randint(int(self.base_size * 0.5), int(self.base_size * 2.0))
            ...
            image = np.asarray(Image.fromarray(np.uint8(image)).resize((w, h), Image.BICUBIC))
            label = cv2.resize(label, (w, h), interpolation=cv2.INTER_NEAREST)
            label2 = cv2.resize(label2, (w, h), interpolation=cv2.INTER_NEAREST)
    ...
    def _crop_tri(self, image, label, label2):
        # Padding to return the correct crop size
        if (isinstance(self.crop_size, list) or isinstance(self.crop_size, tuple)) and len(self.crop_size) == 2:
            crop_h, crop_w = self.crop_size
        elif isinstance(self.crop_size, int):
        ...
        if pad_h > 0 or pad_w > 0:
            image = cv2.copyMakeBorder(image, value=self.image_padding, **pad_kwargs)
            label = cv2.copyMakeBorder(label, value=self.ignore_index, **pad_kwargs)
            label2 = cv2.copyMakeBorder(label2, value=self.ignore_index, **pad_kwargs)
    ...
    def _flip_tri(self, image, label, label2):
        # Random H flip
        if random.random() > 0.5:
            image = np.fliplr(image).copy()
            label = np.fliplr(label).copy()
    ...
    def __getitem__(self, index):
        ...
        # image, label, image_id = self._load_data(index)
        image, label, label_subcls, image_id = self._load_data(index)
        if self.val:
            image, label = self._val_augmentation(image, label)
        elif self.augment:
            # image, label = self._augmentation(image, label)
            image, label, label_subcls = self._augmentation_tri(image, label, label_subcls)
        ...
        if self.val:
            ...
            label = torch.from_numpy(np.array(label, dtype=np.int32)).long()
            label_subcls = torch.from_numpy(np.array(label_subcls, dtype=np.int32)).long()
