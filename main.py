...
from trainer import Test, Save_Features, Trainer_Baseline, Trainer_USRN, Trainer_Ours
...
def main(gpu, ngpus_per_node, config, resume, test, save_feature, visualize,cont_vis):
    if gpu == 0:
        train_logger = Logger()
    else:
        train_logger = None
    ...
    if test:
        ...
        model = models.Test(num_classes=val_loader.dataset.num_classes, conf=config['model'],
                                     sup_loss=sup_loss, ignore_index=val_loader.dataset.ignore_index, cont_vis=cont_vis)
        if gpu == 0:
            print(f'\n{model}\n')
        # TRAINING
        ...
        trainer = Trainer_USRN(
            ...
            gpu=gpu,
            test=test)
    elif config['name'] == 'Ours':
        supervised_loader = sup_dataloader(config['train_supervised'])
        unsupervised_loader = unsup_dataloader(config['train_unsupervised'])
        sup_loss = CE_loss
        model = models.Ours(num_classes=val_loader.dataset.num_classes, conf=config['model'],
                                     sup_loss=sup_loss, ignore_index=val_loader.dataset.ignore_index)
        if gpu == 0:
            print(f'\n{model}\n')
        # TRAINING
        trainer = Trainer_Ours(
            model=model,
            resume=resume,
            config=config,
            supervised_loader=supervised_loader,
...
if __name__ == '__main__':
    ...
    parser.add_argument('-v', '--visualize', default=False, type=bool,
                        help='whether to test')
    parser.add_argument('-cv', '--cont_vis', default=False, type=bool,
                        help='whether to test')
    ...
    mp.spawn(main, nprocs=config['n_gpu'], args=(config['n_gpu'], config, args.resume, args.test, args.save_feature, args.visualize,args.cont_vis))
