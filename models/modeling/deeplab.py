...
from models.modeling.decoder import build_decoder, build_decoder_ours
...
class DeepLab_ours(nn.Module):
    def __init__(self, backbone='resnet101', output_stride=16, num_classes=21,
                 sync_bn=False, freeze_bn=False):
        super(DeepLab_ours, self).__init__()
        if backbone == 'drn':
            output_stride = 8
        ...
        self.aspp_fl = build_aspp(backbone, output_stride, BatchNorm)
    ...
    def forward(self, input):
        x, low_level_feat = self.backbone(input)
        x_ce = self.aspp(x)
        x1 = self.decoder1(x_ce, low_level_feat)

        x_fl = self.aspp_fl(x)
        x2 = self.decoder2(x_fl, low_level_feat)
        # x = F.interpolate(x, size=input.size()[2:], mode='bilinear', align_corners=True)
        return x1, x2
    ...
    def forward_1(self, x, low_level_feat):
        x_ce = self.aspp(x)
        x1 = self.decoder1(x_ce, low_level_feat)
        return x1
    
    def forward_2(self, x, low_level_feat):
        x_fl = self.aspp_fl(x)
        x2 = self.decoder2(x_fl, low_level_feat)
        return x2
    ...
    def get_module_params(self):
        modules = [self.aspp, self.decoder1, self.decoder2]
        for i in range(len(modules)):
            for m in modules[i].named_modules():
                if self.freeze_bn:
                    if isinstance(m[1], nn.Conv2d):
